...
CMy4Dlg::CMy4Dlg(CWnd* pParent /*=nullptr*/)
...
BEGIN_MESSAGE_MAP(CMy4Dlg, CDialogEx)
	...
	ON_BN_CLICKED(IDCANCEL, &CMy4Dlg::OnBnClickedCancel)
	ON_BN_CLICKED(IDC_BUTTON5, &CMy4Dlg::OnBnClickedButton5)
END_MESSAGE_MAP()
...
{
	CColorDialog p;  //ColorDialog불러오는 명령어
	p.DoModal();
...
{
	CFileDialog p(true); //File을 불러오는 명령어
	p.DoModal();
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}
...
{
	CSonDlg p; //아까만든 Son다이얼 로그에 추가한 CSonDlg를 불러오는 코드
	p.DoModal();
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}
...
void CMy4Dlg::OnBnClickedButton4()
{
	CSonDlg2 p;
	if (p.DoModal() == IDOK) {
		m_List.AddString(p.m_Str);
	}
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}


void CMy4Dlg::OnBnClickedCancel()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CDialogEx::OnCancel();
}
...
#include"CModless.h"
void CMy4Dlg::OnBnClickedButton5()
{
	CModless* p = new CModless(); //CModless는 IDD_DLG_MODLESS에 추가한 클래스 이름임.
	p->Create(IDD_DLG_MODLESS);
